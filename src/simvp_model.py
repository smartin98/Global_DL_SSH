# ALL CODE HERE ADAPTED FROM https://github.com/chengtan9907/OpenSTL
...
class SimVP_Model_no_skip(nn.Module):
    ...
    def __init__(self, in_shape, hid_S=16, hid_T=256, N_S=4, N_T=4, model_type='gSTA',
        ...
        super(SimVP_Model_no_skip, self).__init__()
        T, C, H, W = in_shape  # T is pre_seq_length
        H, W = int(H / 2**(N_S/2)), int(W / 2**(N_S/2))  # downsample 1 / 2**(N_S/2)
        act_inplace = False
        self.enc = Encoder_no_skip(C, hid_S, N_S, spatio_kernel_enc, act_inplace=act_inplace)
        self.dec = Decoder_no_skip(hid_S, C, N_S, spatio_kernel_dec, act_inplace=act_inplace)
...
class SimVP_Model_no_skip_sst(nn.Module):
    ...
    def forward(self, x_raw, **kwargs):
        ...
        embed1 = self.enc1(x[:,0,:,:].reshape(B*T,1,H,W))
        embed2 = self.enc2(x[:,1,:,:].reshape(B*T,1,H,W))
